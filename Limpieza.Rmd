...
```{r}
prov_mensual <- df_Turismo %>%
  group_by(Region, Periodo) %>%
  summarise(Total = sum(Total, na.rm = TRUE), .groups = "drop")

ranking_prov <- prov_mensual %>%
  group_by(Region) %>%
  summarise(total_acum = sum(Total, na.rm = TRUE), .groups = "drop")

top5 <- ranking_prov %>% slice_max(total_acum, n = 5)
bottom5 <- ranking_prov %>% slice_min(total_acum, n = 5)

prov_top <- prov_mensual %>% filter(Region %in% top5$Region)
prov_bottom <- prov_mensual %>% filter(Region %in% bottom5$Region)

ggplot(prov_top, aes(Periodo, Total)) +
  geom_line(linewidth = 0.3) +
  facet_wrap(~ Region, scales = "free_y", ncol = 3) +
  scale_y_continuous(labels = label_comma(big.mark = ".")) +
  labs(title = "Serie temporal por provincia: Top 5 con más turismo",
       x = "Periodo", y = "Total") +
  theme_minimal() +
  theme(
    aspect.ratio = 0.65,
...
ggplot(prov_bottom, aes(Periodo, Total)) +
  geom_line(linewidth = 0.3) +
  facet_wrap(~ Region, scales = "free_y", ncol = 3) +
  scale_y_continuous(labels = label_comma(big.mark = ".")) +
  labs(title = "Serie temporal por provincia: Bottom 5 con menos turismo",
       x = "Periodo", y = "Total") +
  theme_minimal() +
  theme(
    aspect.ratio = 0.65,
