...
creamos una columna trimestre que facililite la representacion
```{r}
df_Turismo_Ocupacion %>%
  mutate(Año = factor(sub("Q.", "", as.character(Periodo)))) %>% 
 ggplot(aes(x = Total_turismo, y = Numero_ocupados, color = Trimestre)) + 
  geom_point() +
  facet_wrap(~ Comunidades.y.Ciudades.Autónomas, scales = "free_y") +  # Crea un gráfico por comunidad
  labs(title = "Gráfico de dispersión: Total Turismo vs. Número de Ocupados por Comunidad Autónoma",
       x = "Total Turismo (viajeros + pernoctaciones)",
       y = "Número de Ocupados en el Sector Servicios",
       color = "Año") +
  theme_minimal()
