...
```{r, include=FALSE, out.height="50%", out.width="100%"}
df_Turismo %>%
  mutate(Mes = month(as.Date(Periodo), label = TRUE, abbr = TRUE)) %>% 
  group_by(CCAA, Mes) %>%
  filter(Mes=='jul') %>% 
...
```{r Gráfica de ajuste, out.height="30%", out.width="100%"}
AnalisisBox[[2]]
```
...
```{r Gráfica de predicción buena, echo=FALSE, out.height="30%", out.width="100%"}
AnalisisBox[[18]]
```
...
```{r Gráfica de predicción mala, echo=FALSE, out.height="30%", out.width="100%"}
AnalisisBox[[13]]
```
