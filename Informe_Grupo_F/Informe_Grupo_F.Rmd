...
for (cc in ccaa_lista) {
  p <- outliers_sigma3_est %>%
    ...
    labs(
      title = paste("Detección estacional de outliers (3σ) -", cc),
      x = "Periodo", 
      y = "Total", 
      color = "Outlier"
    ) +
...
##TODO**Base de datos actualizada con los valores imputados**

```{r}
# Definir los periodos COVID que vamos a imputar
covid_inicio <- as.yearmon("2020-01")
covid_fin    <- as.yearmon("2021-12")
...
```{r}
# Definimos el rango de meses a sustituir
inicio_covid <- as.Date("2020-03-01")
fin_covid    <- as.Date("2021-03-30")

# Identificamos los periodos (meses) dentro de ese rango en df_Turismo
periodos_covid <- df_Turismo %>%
  filter(Periodo >= inicio_covid & Periodo <= fin_covid) %>%
  distinct(Periodo) %>%
  arrange(Periodo) %>%
  pull(Periodo)

# Convertimos df de predicciones (12 o más filas, columnas = CCAA) a formato largo
df_pred <- df %>%
  mutate(Periodo = periodos_covid) %>%   # asigna las fechas COVID a las filas del df
  pivot_longer(
    cols = -Periodo,
    names_to = "CCAA",
    values_to = "Total_imputado"
  )

# Sustituimos en df_Turismo los valores originales por los imputados en ese periodo
df_Turismo_corregido <- df_Turismo %>%
  left_join(df_pred, by = c("CCAA", "Periodo")) %>%
  mutate(
    Total = if_else(
      !is.na(Total_imputado),       # si hay predicción
      as.numeric(Total_imputado),   # usamos la imputada
      as.numeric(Total)             # si no, mantenemos la original
    )
  ) %>%
  select(-Total_imputado)
